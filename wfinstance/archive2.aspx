...
<body>
    <form id="form1" runat="server">
        <center>
    <div>
        <input type="hidden" id="Ids" name="Ids" value="<%=Request["Ids"]%>" />
        <input type="hidden" id="ObjectTypeCode" name="ObjectTypeCode" value="<%=Request["ObjectTypeCode"]%>" />
        <table>
            <tr>
                <td>归档时间:</td>
                <td><input type="text" id="ArchivedOn" name="ArchivedOn"  style="width:150px;height:25px;" disabled="disabled"  /></td>
            </tr>
            <tr>
                <td>所属部门</td>
                <td>
                    <input class="easyui-combobox" id="ArchivedDeptName" name="ArchivedDeptName" data-options="url:'/apps/CommandProcessor.ashx?c=1&cmd=workflow.archive.dept.list',method:'get',valueField:'Name',textField:'Name',panelHeight:'100px;'"  />
                </td>
            </tr>
             <tr>
                <td>保管期限</td>
            ...
            <tr>
                <td>文种</td>
                <td>
                    <select id="DocumentTypeCode" name="DocumentTypeCode">
                        <option value=""></option>
        ...
        <div><p>
            <input type="button" id="btnSave"  name="btnSave" value="确认" onclick="validateSave()"  />
            <input type="button" value="取消关闭"   onclick="javascript: closeDialog(false);"  />
             </p>
    ...
    <script>
        ...
        $(function () {
            var day = formatDateTime(new Date());
            $('#ArchivedOn').val(day);
            //var fileDate = getQueryString("CreatedOn");
            //$('#ArchivedOn').val(day); 
        ...
        function validateSave() {
            ...
            var ArchivedOn = document.getElementById("ArchivedOn").value;
            var ObjTypeCode = document.getElementById("ObjectTypeCode").value;
            if (typeof parent.archive2ids != 'undefined') {
                ids = parent.archive2ids
                ObjTypeCode = parent.archive2ObjectTypeCode
            }
            if (Ids == "") return;
            var d = {
                Ids: ids,
                ...
                DocumentTypeCode: document.getElementById("DocumentTypeCode").value,
                RetentionPeriod: document.getElementById("RetentionPeriod").value,
                ArchivedDeptName: document.getElementById("ArchivedDeptName").value
            }
            ajaxMethodPostData("workflow.archive", d, archiveSuccess);
        ...
        var node = parent.$('.tree-node-selected')[0];
        var treenode = parent.$('#leftTree').tree('getData', node);
        if (treenode) {
            if (treenode.id !== '') {
                $('input[name="ArchivedDeptName"]').val(treenode.text)
            }
