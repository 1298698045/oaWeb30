...
function newmsg() {
    ...
    $('.calendar1 .eventList').click(function (e) {
        if (getQueryString('approve')) {
            var result = SundayAllow($(this), year + '-' + mon + '-' + date)
            if (!result) {
                return
            }
        }
        ...
        if (noticeforreading) {
            var nexturl = '/template/alert/new_meetingroommsg.html%3ftime=' + time + '%26day=' + year + '-' + mon + '-' + date + '%26roomid=' + roomid + '%26roomname=' + roomname
            $('#iframe').attr('src', '/template/alert/noticeforreading.html?nexturl=' + nexturl)
        } else {
            $('#iframe').attr('src', '/template/alert/new_meetingroommsg.html?time=' + time + '&day=' + year + '-' + mon + '-' + date + '&roomid=' + roomid + '&roomname=' + roomname);
        }
    ...
    $('.calendar3 td').click(function (e) {
        ...
        if (getQueryString('approve')) {
            var result = SundayAllow($(this).parent(), time)
            if (!result) {
                return
            }
        }
        ...
        if (d2 > d1) {
            ...
            $('#iframe').show();
            if (noticeforreading) {
                var nexturl = '/template/alert/new_meetingroommsg.html%3fday=' + time + '%26roomid=' + roomid + '%26roomname=' + roomname
                $('#iframe').attr('src', '/template/alert/noticeforreading.html?nexturl=' + nexturl)
            } else {
                $('#iframe').attr('src', '/template/alert/new_meetingroommsg.html?day=' + time + '&roomid=' + roomid + '&roomname=' + roomname);
            }
        }
    ...
    function addcalendar2() {
        ...
        $('.newbtn').click(function () {
            $('#iframe').show();
            if (noticeforreading) {
                var nexturl = '/template/alert/new_meetingroommsg.html'
                $('#iframe').attr('src', '/template/alert/noticeforreading.html?nexturl=' + nexturl)
            } else {
                $('#iframe').attr('src', '/template/alert/new_meetingroommsg.html');
...
function getdata() {
    ...
    ajaxMethodGetData('meetingroom.reservation.getlist', d, callback);
} //日视图数据


function daydata(data,time) {
    ...
    for (var i = 0; i < data.listData.length; i++) {
        ...
        var SundayAllow = res.SundayAllow
        var MondayAllow = res.MondayAllow
        var FridayAllow = res.FridayAllow
        var SaturdayAllow = res.SaturdayAllow
        var ThursdayAllow = res.ThursdayAllow
        var TuesdayAllow = res.TuesdayAllow
        var WednesdayAllow = res.WednesdayAllow

        var day = new Date(time).getDay()
        var days = ['SundayAllow', 'MondayAllow', 'TuesdayAllow', 'WednesdayAllow', 'ThursdayAllow', 'FridayAllow', 'SaturdayAllow']
        var classname = ''
        if (res[days[day]]==false) {
            classname = 'cannotorder'
        }
        $('.calendar1').find('.calendarDayHeaders').append("<strong class=\"calenderDayHeader\" >" + name + "</strong>");
        $('.calendar1').find('.rowCore .scroll').append("<div class=\"calendarDay \">\n        <div class=\"eventList " + classname + "\" MondayAllow=\"" + MondayAllow + "\" FridayAllow=\"" + FridayAllow + "\" SaturdayAllow=\"" + SaturdayAllow + "\" WednesdayAllow=\"" + WednesdayAllow + "\" TuesdayAllow=\"" + TuesdayAllow + "\" ThursdayAllow=\"" + ThursdayAllow + "\" SundayAllow=\"" + SundayAllow + "\" roomname = \"" + name + "\"id=\"" + id + "\"roomid=\"" + id + "\"></div>\n            </div>");
...
function weekdata(data) {
    ...
    for (var i = 0; i < data.listData.length; i++) {
        ...
        var SaturdayAllow = res.SaturdayAllow
        var html = ''
        if (SundayAllow == false) {
            html += '<td class="cannotorder"></td>'
        } else {
            html += '<td class=""></td>'
        }
        if (MondayAllow == false) {
            html += '<td class="cannotorder"></td>'
        } else {
            html += '<td class=""></td>'
        }
        if (TuesdayAllow == false) {
            html += '<td class="cannotorder"></td>'
        } else {
            html += '<td class=""></td>'
        }
        if (WednesdayAllow == false) {
            html += '<td class="cannotorder"></td>'
        } else {
            html += '<td class=""></td>'
        }
        if (ThursdayAllow == false) {
            html += '<td class="cannotorder"></td>'
        } else {
            html += '<td class=""></td>'
        }
        if (FridayAllow == false) {
            html += '<td class="cannotorder"></td>'
        } else {
            html += '<td class=""></td>'
        }
        if (SaturdayAllow == false) {
            html += '<td class="cannotorder"></td>'
        } else {
            html += '<td class=""></td>'
        }
        $('#zhoucaltable').append('<tr MondayAllow=\"' + MondayAllow + '\" FridayAllow=\"' + FridayAllow + '\" SaturdayAllow=\"' + SaturdayAllow + '\" WednesdayAllow=\"' + WednesdayAllow + '\" TuesdayAllow=\"' + TuesdayAllow + '\" ThursdayAllow=\"' + ThursdayAllow + '\" SundayAllow="' + SundayAllow + '" roomname="' + name + '" roomid="' + id + '"><td>' + name + '</td>'+html+'</tr>')
        for (var j = 0; j < res.Reserves.length; j++) {
            var ress = res.Reserves[j];
            var ActionsConfig = ress.Room.ActionsConfig
            var canApprove = ActionsConfig.canApprove
...
function mondata(data) {
    ...
    for (var i = 0; i < data.listData.length; i++) {
        ...
        var TuesdayAllow = res.TuesdayAllow
        var WednesdayAllow = res.WednesdayAllow
        $('#yuecaltable').append('<tr MondayAllow=\"' + MondayAllow + '\" FridayAllow=\"' + FridayAllow + '\" SaturdayAllow=\"' + SaturdayAllow + '\" WednesdayAllow=\"' + WednesdayAllow + '\" TuesdayAllow=\"' + TuesdayAllow + '\" ThursdayAllow=\"' + ThursdayAllow + '\" SundayAllow="' + SundayAllow + '" roomname="' + name + '" roomid="' + id + '"><td style="padding:5px;background-color:#f5f6f9" width="69">' + name + '</td></tr>')
        $('.table-flexleft').append('<div style="width:81px;">' + name + '</div>')
        $('.calendar2 .calendarDayHeaders strong').each(function (index, dom) {
            var day = $(this).find('p').eq(0).html()
            var number = 0
            switch (day) {
                case '日':
                    number = 0
            ...
            var days = ['SundayAllow', 'MondayAllow', 'TuesdayAllow', 'WednesdayAllow', 'ThursdayAllow', 'FridayAllow', 'SaturdayAllow']
            var classname = ''
            if ($('#yuecaltable tr:last').attr(days[number]) == 'false') {
                classname = 'cannotorder'
            }
            $('#yuecaltable tr:last').append('<td width="49" class="'+classname+'"></td>')
        })
        for (var j = 0; j < res.Reserves.length; j++) {
            var ress = res.Reserves[j];
            var ActionsConfig = ress.Room.ActionsConfig
...
$('.meetingroom-message-hover-foot-right .buttonbtn').eq(2).click(function () {
    ...
    $('#iframe').attr('src', '/template/alert/new_meetingroommsg.html?msgid=' + Id);
    setTimeout(function () {
        $('#iframe').show();
    }, 100);
}); //删除
...
$('.meetingroom-message-hover-foot-right .buttonbtn').eq(3).click(function () {
    ...
    $('#iframe').attr('src', '/template/alert/delete_meetingroommsg.html?msgid=' + Id + '&name=会议室预约');
    setTimeout(function () {
        $('#iframe').show();
    }, 100);
});
...
function SundayAllow(dom,time) {
    var days = ['SundayAllow', 'MondayAllow', 'TuesdayAllow', 'WednesdayAllow', 'ThursdayAllow', 'FridayAllow', 'SaturdayAllow']
    if ($(dom).attr(days[new Date(time).getDay()]) == 'false') {
        alert('该会议室此日期不可预约')
        return false
    }
    return true
