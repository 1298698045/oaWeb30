...
function weekdata(data) {
    var height = data.listData.length * 100;
    ...
    for (var i = 0; i < data.listData.length; i++) {
        ...
        for (var j = 0; j < res.Reserves.length; j++) {
            ...
            var top = parseInt($('.calendar3').find('.calendarKey strong:last').offset().top - 228 + $('.calendar3 .rowCore').scrollTop()) + 'px';
            var morenumber = 0;
            var morenumber2 = 0;
            ...
            $('.calendar3').find('.rowCore .eventList').eq(index).find('div').each(function () {
                if ($(this).css('top') == top) {
                    morenumber++;
                    top = parseInt($('.calendar3').find('.calendarKey strong:last').offset().top - 228 + 40 + $('.calendar3 .rowCore').scrollTop()) + 'px';
                    var moretop = parseInt($('.calendar3').find('.calendarKey strong:last').offset().top - 228 + 80 + $('.calendar3 .rowCore').scrollTop()) + 'px';

                    if (morenumber > 1) {
                        morenumber2 = morenumber - 1;
                        $(this).parent().find('.more').remove();
                        $(this).parent().append("<div BusinessApproveStateCode=\"" + BusinessApproveStateCode + "\" canBusinessUnitApprove=\"" + canBusinessUnitApprove + "\" canApprove = \"" + canApprove + "\"\n  ResourceId='" + id + "'class='more'style='top:" + moretop + "'>\u66F4\u591A</div>");
                    }
                }
            });
...
function mondata(data) {
    ...
    for (var i = 0; i < data.listData.length; i++) {
        ...
        for (var j = 0; j < res.Reserves.length; j++) {
            ...
            var top = parseInt($('.calendar2').find('.calendarKey strong:last').offset().top - 228 + $('.calendar2 .rowCore').scrollTop()) + 'px';
            var morenumber = 0;
            $('.calendar2').find('.rowCore .eventList').eq(index).find('div').each(function () {
                if ($(this).css('top') == top) {
                    morenumber++;
                    var morenumber2 = 0;
                    top = parseInt($('.calendar2').find('.calendarKey strong:last').offset().top - 228 + 40 + $('.calendar2 .rowCore').scrollTop()) + 'px';
                    var moretop = parseInt($('.calendar2').find('.calendarKey strong:last').offset().top - 228 + 80 + $('.calendar2 .rowCore').scrollTop()) + 'px';

                    if (morenumber > 1) {
                        morenumber2 = morenumber - 1;
                        $(this).parent().find('.more').remove();
                        var number = res.Reserves.length;
                        $(this).parent().append("<div BusinessApproveStateCode=\"" + BusinessApproveStateCode + "\" canBusinessUnitApprove=\"" + canBusinessUnitApprove + "\" canApprove = \"" + canApprove + "\"\n    <div isShareable = \"" + isShareable + "\"\n isOwner = \"" + isOwner + "\"\n isEditable = \"" + isEditable + "\"\n isDeletable = \"" + isDeletable + "\"\n canShow = \"" + canShow + "\"\n <div ResourceId='" + id + "'class='more'style='top:" + moretop + "'>+" + morenumber2 + " \u66F4\u591A</div>");
                    }
                }
            });
