...
$('.Pinned').remove();
$('.noPinned').remove();
//$('.forceListViewPicker').after(`<div class="fixed-list Pinned">
...
$('#print').click(function () {
    ...
    window.open('/apps/CommandProcessor.ashx?cmd=filter.exportfile&id=' + id)
})
$('.shaixuan').off();
$('.tubiao').off();
$('.shaixuan').off();
            ...
            for (var i = 0; i < cond.length; i++) {
                ...
                if (cond[i].name == $(this).find('.sectionable-table-row-containers').attr('attribute')) {
                    ...
                    if (dType == 'Text') {
                        ...
                        $('#Text2').val($(this).find('.sectionable-table-row-containers').attr('picklistvalues'))
                        ...
                        for (var j = 0; j < res.filterValues.length; j++) {
                            var filterValues = res.filterValues[j];
                            var name = filterValues.name;
                            var label = filterValues.label;
                            $('.slds-combobox__form-element').eq(2).next('.focuss').find('ul').append("\n                                             <li class=\"slds-listbox__item picklist\" name=\"" + name + "\">\n                                                <div class=\"focus_title\">\n                                                <i class=\"iconfont icon-duihao\" style=\"opacity: 0\"></i>\n                                                    <span class=\"slds-media__body\">" + label + "</span>\n                                                    \n                                                </div>\n                                             </li>");
                        }
                        $('#Text2').val($(this).find('.sectionable-table-row-containers').attr('picklistvalues'))
                        $('#Text2').attr('name', $(this).find('.sectionable-table-row-containers').attr('value'))
                    } else if (dType == 'DateTime' || dType == 'Date') {
                        var time = $(this).find('.sectionable-table-row-containers').attr('picklistvalues')
                        if (time.indexOf(',') != -1) {
                            var time1 = time.split(',')[0]
                            var time2 = time.split(',')[1]
                            $('.el-input__inner').eq(0).val(time1)
                            $('.el-input__inner').eq(1).val(time2)

                        } else {
                            $('.el-input__inner').val(time)
...
gridFilterDesinger.prototype.addCondition = function (cond) {
    ...
    $('.addFilter').click(function () {
        ...
        $('.slds-popover__body input').not('input[type=button]').val('')
        $('.valuediv>div').hide()
        $('.valuediv .value').show()
        $('.valuediv #vueapp').show()
        $('#vueapp .timechook').hide()
    });
    //完成
    ...
    $('.filteredit-finish').click(function () {
        ...
        } else if ($('.buttoncheck').css('display') != 'none') {
...
gridFilterDesinger.prototype.getFilterExpression = function () {
    var url = "/apps/CommandProcessor.ashx?cmd=entity.filter.get&id=" + this.filterId;
    var self = this;
    jQuery.ajax({
        async: false,
        cache: true,
        ...
        success: function success(data, textStatus) {
            if (data) {
                ...
                if (filter.length > 0) {
                    ...
                    var IsDefault = filterExp.IsDefault
                    if (IsDefault == 'True') {
                        $('.Pinned').show()
                        $('.noPinned').hide()
                    }
...
gridFilterDesinger.prototype.getFilter = function () {
    ...
    $('ul.section-table-rows-container .sectionable-table-row').each(function () {
        logical = $(this).find('.sectionable-table-row-containers').attr('logical');
        ...
        picklistValue = $(this).find('.sectionable-table-row-containers').attr('picklistvalues'); //var sort = $(this).index()
...
$('#exportOrgs').click(function () {
    $('#iframe').attr('src', '/alert/filter/new.html?type=newfilter3');
    $('#changeDiv').show();
    $('.popup-mask').show()
    $('#iframe').show();
});
$('#ryz8MyRxN').keyup(function () {
    var search = $(this).val()
    $(this).parent().next().find('li').each(function () {
        if ($(this).find('.slds-media__body').html().indexOf(search) == -1) {
            $(this).hide()
        } else {
            $(this).show()
