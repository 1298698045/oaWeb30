...
function getdata() {
    ...
    function callback(data) {
        ...
        data.listData = data.returnValue.records;
        daydata(data);
        renderdata(data);
        weekdata(data);
        mondata(data);
    ...
    ajaxMethodGetData('vehicle.reservation.getlist', d, callback);
} //日视图数据
...
function daydata(data) {
    ...
    for (var i = 0; i < data.listData.length; i++) {
        ...
        for (var j = 0; j < res.Reserves.length; j++) {
            ...
            var canBusinessUnitApprove = '';
            var BusinessApproveStateCode = '';
            var canApprove = '';
            var isShareable = '';
            var isOwner = '';
            var isShareable = '';
            ...
            var Roomname = ress.Name;
            var Id = ress.Id;
            var OwningUserName = ress.OwningUserName;
            var OwningUser = ress.OwningUser;
            var Subject = name;
            var StateCode = ress.StateCode;
            var Attachs = ress.VehicleIdName;
            var TelePhone = '';
            var Description = ress.Description;
            var ScheduledEnd = ress.EndTime.split('T')[1];
            var ScheduledStart = ress.StartTime.split('T')[1];
            var ScheduledEnd2 = ress.EndTime.split('T').join(' ');
            var ScheduledStart2 = ress.StartTime.split('T').join(' ');
            var ScheduledStart3 = ress.StartTime.split('T')[0];
