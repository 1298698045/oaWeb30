...
function newmsg() {
    ...
    $('.calendar1 .eventList').click(function (e) {
        ...
        $('#iframe').show();
        $('#changeDiv').show();
        $('#iframe').attr('src', '/alert/meeting/add_meeting.html?time=' + time + '&day=' + year + '-' + mon + '-' + date);
        //console.log(time);
    }); //新建 周
    ...
    $('.calendar3 .eventList').click(function (e) {
        ...
        $('#iframe').attr('src', '/alert/meeting/add_meeting.html?day=' + data + '&time=' + time);
    }); //新建 月
    ...
    $('.calendar2 .scroll>div').click(function (e) {
        ...
        $('#iframe').show();
        $('#changeDiv').show()
        $('#iframe').attr('src', '/alert/meeting/add_meeting.html?day=' + time);
    });
...
function weekdata(data) {
    ...
    for (var j = 0; j < res.meetingItems.length; j++) {
        ...
        if (startday == endday) {
            $('.calendar3').find('.rowCore .eventList').eq(index).append("\n            <div \n     isEditable='" + isEditable + "'isDeletable='" + isDeletable + "'       ApproveByIds = \"" + Id + "\"\n            TelePhone=\"" + TelePhone + "\"\n            Description=\"" + Description + "\"\n            Attachs=\"" + Attachs + "\"\n            Subject = \"" + Subject + "\"\n            ScheduledEnd = \"" + ScheduledEnd2 + "\"\n            ScheduledStart = \"" + ScheduledStart2 + "\"\n            OwningUserName =\"" + name + "\" \n            Id=\"" + Id + "\"\n            class=\"meetingroom-message widthitem2\" \n            style='top:" + top + "px;height:" + height + "px;background-color:#" + color + "'>\n                <p>" + Subject + "</p>\n                <p><span>" + ScheduledStart + "</span>-<span>" + ScheduledEnd + "</span></p>\n                <p>" + name + "</p>\n            </div>");
        } else {
            height = 59
...
function mondata(data) {
    ...
    for (var i = 0; i < data.returnValue.meetings.length; i++) {
        ...
        for (var j = 0; j < res.meetingItems.length; j++) {
            ...
            var Subject = ress.Subject;
            var Roomname = ress.Location + ' ' + ress.RoomIdName;
            ...
            $('.calendar2 .scroll>div').each(function () {
                ...
                if (key == 1) {
                    ...
                    if ($(this).find('div').length < 2) {
                        $(this).append("\n                           <div\n       isEditable='" + isEditable + "'isDeletable='" + isDeletable + "'      Roomname = \"" + Roomname + "\"\n              ApproveByIds = \"" + Id + "\"\n                           TelePhone=\"" + TelePhone + "\"\n                           Description=\"" + Description + "\"\n                           Attachs=\"" + Attachs + "\"\n                           Subject = \"" + Subject + "\"\n                           ScheduledEnd = \"" + ScheduledEnd2 + "\"\n                           ScheduledStart = \"" + ScheduledStart2 + "\"\n                           OwningUserName =\"" + name + "\" \n                           Id=\"" + Id + "\"\n                           class=\"meetingroom-message widthitem2\"\n                           style='background-color:#" + Color + "'>\n                           <p>" + Subject + "</p>\n                           <p><span>" + ScheduledStart + "</span>-<span>" + ScheduledEnd + "</span>\n                           " + name + "\u5206\u914D\u7684</p>\n                       </div>");
                    } else {
...
$('.meetingroom-message-hover-foot-right div').eq(0).click(function () {
    ...
    $('#changeDiv').show();
    var Id = $(this).attr('id');
    $('#iframe').attr('src', '/alert/meeting/add_meeting.html?msgid=' + Id);
}); //删除
...
$('.meetingroom-message-hover-foot-right div').eq(1).click(function () {
    ...
    $('#iframe').attr('src', '/alert/delete.html?id=' + Id + '&objTypeCode=5000');
});
