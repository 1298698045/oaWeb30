...
var copyData = {}
{
    ...
    attendanceEditor.prototype.unitType = 4030; //10 dept 30041 attendgroup
    ...
    attendanceEditor.prototype.renderShiftBar = function (data) {
        window.AttendTypesData = data.Shifts;
        ...
        for (var i = 0; i < this.shifts.length; i++) {
            ...
            html += " <button id=\"leavetype_" + item.WorkShiftId + "\" leavetype=\"" + item.WorkShiftId + "\"  title='" + item.Name + "' onclick=\"pickLeaveType('" + item.WorkShiftId + "');\" num='" + i + "'>" + item.Name + "</button>";
        }
    ...
    attendanceEditor.prototype.renderMonthCalendar = function (d) {
        ...
        var data = d.EmpShiftData[0].Employees;
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            for (var h = 0; h < days; h++) {
                ...
                var cellTitle = emp.Name + " " + row.WorkDay;
                var cellId = "c_" + emp.EmployeeId + "_" + row.WorkDay;
                //var memoTxtId = "memo_" + emp.EmployeeId + "_" + row.AttendDate;
                html2 += "<td  id=\"" + cellId + "\" title=\"" + cellTitle + "\"";
                ...
                if (this.viewMode == 1) {
                    ...
                    if (row.ShiftName) {
                        if (row.ShiftName != "") {
                            ...
                            if (totalDays < 1)
                                html2 += "<div style='cursor:pointer;color:grey;' title=\"" + cellTitle + "\" id='c_" + emp.EmployeeId + "_" + row.WorkDay + "_s'> " + shiftName + " </div>";
                        }
                    }
                    ...
                    if (isEditable)
                        html2 += " onclick=\"setEmpWorkStatus('" + emp.EmployeeId + "','" + row.WorkDay + "','0','c_" + emp.EmployeeId + "_" + row.WorkDay + "',this)\"  ";

                    html2 += ">";
                    html2 += "<span class=\"spancolo1\">";
                    html2 += "</span>";
                    for (var j = 0; j < row.dayShifts.length; j++) {
                        //0 工作 1 请假 2休息
                        //debugger;white-space:nowrap;
                        var shift = row.dayShifts[j];
                        ...
                        var BusinessUnitIdName = shift.BusinessUnitIdName
                        var CreatedByName = shift.CreatedByName
                        var GroupIdName = shift.GroupIdName
            ...
            $(".table1 tr").eq(i + 1).append(html2);
            $(".table1-flex tr").eq(i + 1).append(html2);
            $(".table1-flex tr").eq(i + 1).find('td').each(function () {
    ...
    attendanceEditor.prototype.loadDesigner = function (para) {
        ...
        this.loadData("workshift.designer.init", para, function (data) {
            ...
            if (data.CloseStatusCode == 1 || data.closePeriod)//如果封存
            {
                ...
                else if (data.Group.ShiftStatusCode == 1) {
                    showButtons(0);
            ...
            if (data.Group.ShiftStatusName) {
                document.getElementById("attendStatusName").innerHTML = data.Group.ShiftStatusName;
            }
            if (self.searchMode) {
            ...
            ajaxMethodGetData('workshift.dept.month.remark.get', {
                YearNumber: yNumber,
                MonthNumber: mNumber,
                groupId: unitId,
                unitId: unitId,
    ...
    attendanceEditor.prototype.resizeDesigner = function () {
        $(document).ready(function () {
            ...
            //}
            var height = $(window).height() - 100 - $('.attendance-body').height() - $('.attendance-head').height() - 80;
            if (window.isFullScreen) {
                height = $(window).height() - $('.attendance-body').height() - $('.attendance-head').height() - 80;
            }
            
            $('#table-content').css('height', height + 'px');
            $('.attendance-body-table-flex').width($('#attendance').width()-17);
            hover1()
            hover2()
        })
    }
...
function shiftChange() {
    var name = $('#Name').val();
    $('#Name').val(name.replace(/\(.*?\)/g, '') + '(' + $('#PostCategory').val() + ')')
}
function hover2() {
...
function printAttenanceCalendar() {
    ...
    var url = "/_ui/hr/attend/empshift/print?objectTypeCode=30041&md0=" + attendYear + "&md1=" + attendMonth + "&md2=0&unitId=" + unitId + "&calendar=" + 1 + "&showMemo=" + 0;
...
function setEmpWorkStatus(empId, day, oldStatus, eleID,obj) {
    ...
    if (workshiftId == "copy") {
        ...
        if (copyData.copyStartDate) {
            ...
            copyEmployeeDayAttendance2(empId, day, workshiftId, businessUnitId, businessUnitId, eleID, refreshAttendCell, businessUnitId, businessUnitId);
            changedStatus = true;
        }
        else {
...
function openBatchAttend() {
    ...
    var url = "/hr/attend/CreateEmpWorkShift.aspx?objectTypeCode=30041&unitId=" + unitId + "&empId=" + empIds + "&empName=" + empNames;
...
function empTransfer(type) {
    ...
    showDialog(url, "TransferEmp", title, 760, 480);
}
...
function sortEmpDialog() {
    ...
    var url1 = '/attd/shifts/sortGroupMember.aspx?objectTypeCode=30041&unitId=' + unitId + "&unitName=" + unitName + "&yearNumber=" + attendYear + "&monthNumber=" + attendMonth + "&startDate=" + startDate + "&endDate=" + endDate;
...
function pickWorkshift() {
    ...
    showDialog(url1, "Pickerworkshift", "设置使用班次", 1000, 520);
}
function exchangeEmployeeShiftDialog() {
    ...
    var startDate = editor.startDate;
    var endDate = editor.endDate;
    var url = "/attd/shifts/exchangeShift.aspx?objectTypeCode=30041&unitId=" + unitId + "&startDate=" + startDate + "&endDate=" + endDate;
...
function groupArrangeWorkshiftDialog() {
    ...
    if (selectCount <1) {
        ...
        var url = "/attd/shifts/arrangeGroupWorkshift.aspx?startDate=" + sDate + "&endDate=" + eDate + "&year=" + yearNumber + "&month=" + monthNumber + "&objectTypeCode=30041&unitId=" + unitId + "&focus=aboutTab&retURL=%2F_ui%2Fhr%2FGroup%2FWorkshift%2FWeekPage%3Fmd0%3D" + yearNumber + "%26md2%3D" + weekNumber;
...
function batchArrangeWorkshiftDialog() {
    ...
    if (selectCount <1) {
        ...
        var sDate = "";// document.getElementById("startDate").value;
        var eDate = "";//document.getElementById("endDate").value;
        var url = "/hr/attend/CreateEmpWorkShift.aspx?startDate=" + sDate + "&endDate=" + eDate + "&year=" + yearNumber + "&month=" + monthNumber + "&md2=" + weekNumber + "&objectTypeCode=30041&unitId=" + unitId + "&groupId=" + unitId + "&retURL=%2f_ui%2fhr%2fGroup%2fWorkshift%2fWeekPage";
...
function templateArrangeWorkShiftDialog() {
    ...
    if (unitId != undefined) {
        ...
        url += "&objectTypeCode=30041&unitId=" + unitId;
...
function remarkDialog() {
    ...
    var attendYear = jQuery("#yearNumber").val();
    var attendMonth = jQuery("#monthNumber").val();
    var url1 = '/attd/shifts/RemarkWorkshift.aspx?objectTypeCode=30041&unitId=' + unitId + "&unitName=" + unitName + "&yearNumber=" + attendYear + "&monthNumber=" + attendMonth;
