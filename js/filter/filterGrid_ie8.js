...
function newfilter() {
    ...
    function save() {
        ...
        function callback(data) {
            ...
            parent.$('.shaixuan').addClass('active')
            parent.$('.filterbox').show()
            parent.$('.shaixuan').find('img').attr('src', '/img/cal/22.filterList_3(14x14).png')
            parent.$('#gridContainer').width(parent.$('body').width() - 239 - 358)
...
function Rename() {
    ...
    function save(filterid) {
        ...
        parent.$('.header-left-title-listname span').html(name)

        ajaxMethodPostData('entity.filter.rename', d, callback);
    }
} //共享设置

function usercntentchange() {
    var visibility = $('input[type="radio"]:checked').val();
...
function getShare() {
    var filterid = parent.$('.header-left-title-listname.selectinput span').attr('filterid');
    function callback(data) {
        var list = data.actions[0].returnValue?data.actions[0].returnValue.currentShares:'';
        if (list && list.length) {
            $('.orderedList').html('')
            for (i = 0; i < data.length; i++) {
                $('.orderedList').append("<li class=\"pillContainerListItem\" id=\"" + $('#employeeId').attr('data-object') + ":" + list[i].id + "\">\n                             <a class=\"pill uiPill--default uiPill\" href=\"#\">\n                                 <span class=\"pillIcon\" style=\"background-color: #65CAE4\">\n                                     <span class=\"uiImage\">\n                                         <img src=\"https://ap8.salesforce.com/img/icon/t4v35/standard/user_120.png\" alt=\"\u7528\u6237\">\n                                     </span>\n                                 </span>\n                                 <span class=\"pillText\">" + list[i].name + "</span>\n                                 <span class=\"pilldelete\">\xD7</span>\n                             </a>\n                         </li>");
            }
            $('.lookup').hide()
            ...
            $('.pillContainerWrapper.noSeparator.people_show').show()
        }
        var visibility = data.actions[0].returnValue ? data.actions[0].returnValue.visibility : '';
        if (visibility == 'SHARED') {
            $('#radio-304').attr('checked', 'checked')
            $('.slds_section__content_div_col_element').show()
        } else if (visibility == 'GLOBAL') {
            $('#radio-303').attr('checked', 'checked')
            $('.slds_section__content_div_col_element').hide()
        } else if (visibility == 'PRIVATE') {
            $('#radio-302').attr('checked', 'checked')
            $('.slds_section__content_div_col_element').hide()
        }
    }
    ajaxMethodGetData('listview.share.getlist&id=' + filterid, null, callback)
}
function enjoy() {
    var name = parent.$('.header-left-title-listname.selectinput span').html();
    var filterid = parent.$('.header-left-title-listname.selectinput span').attr('filterid');
