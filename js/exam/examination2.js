...
function evalExamRender(eleId, evaluationId, evaluationFormId) {
...
evalExamRender.prototype.renderHeader = function (data) {
    ...
    html += '<th colspan="3"rowSpan="2"class="gridheader">统计</th>'
    html += ("</tr>");
    html += ("<tr>");
    ...
    html += '<th class="gridheader">评分人数</th>'
    html += '<th class="gridheader">总数</th>'
    html += '<th class="gridheader">平均分</th>'
    html += ("</tr>");
    this.loadEvalObjects();
    html += this.renderEvalObjects();
    html += "</table>";
    html += ("<div class='error' id='errorBottom'></div>");
    html += "<div class='pbButtonb ' ><input type='button' name='btnDraft' value='  保存  ' class='btnSecondary' onclick=\"evalRender.saveData(1);\">&nbsp;<input type='button' name='btnSubmit' value='  提交  ' class='btnPrimary'  onclick=\"evalRender.saveData(3);\"></div>";
    html += "</div>";
    $("#" + this.elementId).append(html);
    //$("#" + this.elementId).append(html);
}
