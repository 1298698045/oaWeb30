{
    ...
    attendanceEditor.prototype.renderMonthViewCalendar = function (d) {
        //debugger;
        var html = "<table style='font-size:13px;' width=\"100%\" cellpadding=\"0\" cellspacing=\"0\"><tr><td><span> 科室：</span><b>" + d.UnitName + "</b></td><td></td>";
        html += "<td><b>" + d.Year + "</b>年<b>" + d.MonthNumber + "</b>月 </td><td></td><td style='text-align:right;'> <span> 状态:</span> <b>" + d.GroupReportStatusName + "</b></td></tr></table>";
...
function printShiftCalendar() {
    ...
    //printWin(url);
    window.open(url, "打印部门排班", 'titlebar=yes,fullscreen=yes,top=0,left=0,width=1200,height=900,resizable=yes,scrollbars=yes,menubar=yes');
}
/*打印考勤明细*/
...
function openBatchAttend() {
    ...
    for (var i = 0; i < $('#dg').datagrid('getChecked').length; i++) {
        empIds.push($('#dg').datagrid('getChecked')[i].EmployeeId);
        empNames.push($('#dg').datagrid('getChecked')[i].EmployeeName);
    }
    empIds = empIds.join(',')
