...
var copyData = {}
{
    ...
    attendanceEditor.prototype.rendertable = function () {
        ...
        if (node.isleft) {
            ...
            prea += "&DeptId=" + node.ParentId
            prea += "&BusinessUnitId=" + node.ParentId
        }
        //prea += '&pagesize=' + $('#pagesizecheck').val()
        //prea += '&page=' + $('#pageto').val()
        var that = this
        this.loadData("workshift.deptgroup.employee.week.getlist", prea, function (data) {
    ...
    attendanceEditor.prototype.renderFixColumns = function (d) {
        ...
        for (var j = 0; j < groups.length; j++) {
            for (var i = 0; i < groups[j].data.length; i++) {
                ...
                if (i != 0) {
                    if ((i % 2) == 0) {
                        html2 += "<tr empid='" + emp.EmployeeId + "' groupid='" + groups[j].GroupId + "' sortNumber='" + sortNumber + "'>";
                    }
                    else {
                        html2 += "<tr empid='" + emp.EmployeeId + "' groupid='" + groups[j].GroupId + "' sortNumber='" + sortNumber + "' class=\"trcolo\">";
                    }
                } else {
                    if ((i % 2) == 0) {
                        html2 += "<tr empid='" + emp.EmployeeId+ "' sortNumber='" + sortNumber + "' class='grouprow'>";
                    }
                    else {
                        html2 += "<tr empid='" + emp.EmployeeId + "' sortNumber='" + sortNumber + "'  class=\"trcolo grouprow\">";
                    }
                    html2 += "<td class='flextd' style=\"width: 70px\" rowspan='" + groups[j].data.length + "'><div groupid='" + groups[j].GroupId + "' onclick='updown(this)' class='updowncontent'>" + groups[j].GroupName + "<span class='downbtn'><img src='/img/cal/147.png'/></span></div><div><img onclick='groupappendpeople(" + '"' + groups[j].GroupId + '"' + "," + '"' + UnitId + '"' + ")' class='addpeople' src='/img/cal/13.add_1(16x16).png' /></div></td>";
                ...
                html2 += "<td class=\"nameCol1\" nowrap='nowrap' style=\"vertical-align:middle;\">" + emp.Name + "</td>";
                html2 += "<td class=\"nameCol1\" style=\"vertical-align:middle;\">" + emp.EmployeeNo + "</td>";
                //html2 += '<td style="width:150px"><input onchange=saveDescription(this.value,"' + emp.EmployeeId + '") style="width:calc(100% - 12px);border:1px solid #dedede;margin-left:5px" value="' + (emp.Description||'') + '" /></td>'
                var empInDate = "";
                if (emp.InDate)
                ...
                html2 += '<td>' + (emp.TotalWeekHours || '') + '</td>'
                html2 += '<td>' + (emp.SurplusHours||'') + '</td>'
                html2 += '<td>' + (emp.TotalMonthSurplusHours || '') + '</td>'
                //print mode display stat
                if (this.viewMode == 1) {
                    //html2 += "<td class=\"\" style=\"text-align:center;vertical-align:middle;\">" + emp.AbsentDays + "</td>";
                    if (emp.StatDesc) {
    ...
    attendanceEditor.prototype.loadDesigner = function (para) {
        ...
        this.loadData("workshift.month.deptgroup.headerinfo.get", para, function (data) {
            var html = "";
            if (!data) return;
            //debugger;
            ...
            if (self.searchMode) {
                ...
                document.getElementById("calendarHeader").innerHTML = $('#weektime').val().split('~')[0] + " 至 " + $('#weektime').val().split('~')[1] + "&nbsp;&nbsp;" + data.UnitName + " 人员排班表";
            }
            ...
            {
                document.getElementById("calendarHeader").innerHTML = $('#weektime').val().split('~')[0] + " 至 " + $('#weektime').val().split('~')[1];
            ...
            var yNumber = $('#weektime').val().split('-')[0];
            var mNumber = $('#weektime').val().split('-')[1];

            ajaxMethodGetData('workshift.dept.week.get', {
                YearNumber: yNumber,
                MonthNumber: mNumber,
                unitId: deptId,
                groupId: ""
    ...
    attendanceEditor.prototype.loadtree = function (Units) {
        ...
        for (var i = 0; i < Units.length; i++) {
            data.push({
                children: [],
                TextName:Units[i].Name,
                text: Units[i].Name + (Units[i].ShiftStatusName ? (Units[i].ShiftStatus == '1' ? '<span class="deptzhuangtai" style="color:#108af9;">（' + Units[i].ShiftStatusName + '）</span>' : '<span class="deptzhuangtai">（' + Units[i].ShiftStatusName + '）</span>') : '<span class="deptzhuangtai"></span>'),
                id: Units[i].Id,
                isleft: true
            })
            for (var j = 0; j < Units[i].Groups.length; j++) {
                data[i].children.push({
                    children: [],
                    TextName: Units[i].Groups[j].Name,
                    text: Units[i].Groups[j].Name,
                    id: Units[i].Groups[j].Id,
                    isleft: false,
                    ParentId: Units[i].Groups[j].ParentId,
                    ParentIdName: Units[i].Groups[j].ParentIdName
                })
            }
        }
        //console.log(data)
        ...
        setTimeout(function () {
            if (!$('#LeftTree').tree('getSelected')) {
                var node = $('#LeftTree').tree('find', data[0].id);
    ...
    attendanceEditor.prototype.previous = function () {
        ...
        para += "&startDate=" + $('#weektime').val().split('~')[0];
        para += "&endDate=" + $('#weektime').val().split('~')[1];
        //para += "&md0=" + this.year;
    ...
    attendanceEditor.prototype.resizeDesigner = function () {
        $(document).ready(function () {
            ...
            //}
            var height = $(window).height() - 100 - $('#attendSettings').height() - $('.attendance-head').height()-40;
            if (window.isFullScreen) {
...
function getRemark() {
    ...
    var yNumber = $('#weektime').val().split('-')[0]
    var mNumber = $('#weektime').val().split('-')[1]
    var dNumber = $('#weektime').val().split('~')[0].split('-')[2]
    var Description = document.getElementById("description").value;
    var LeaveMessage = $("#LeaveMessage").val();
    var startDate = $('#weektime').val().split('~')[0]
    var endDate = $('#weektime').val().split('~')[1]
    var WeekNumber = getWeekNumber(yNumber, mNumber, dNumber);
    ...
    ajaxMethodGetData('workshift.dept.week.get', d, function (data) {
        //debugger;
        jQuery('#LeaveMessage').val(data.leaveMessage)
...
function searchMonthEmpAttendances() {
    var selEle = $('#weektime').val().split('-')[0];
    if (selEle)
        editor.year = $('#weektime').val().split('-')[0];

    selEle = $('#weektime').val().split('-')[1];
    if (selEle)
        editor.month = $('#weektime').val().split('-')[1];
...
function printAttenanceCalendar() {
    ...
    var unitName = node.TextName;
    var showm = "0";
    var attendYear = $('#weektime').val().split('-')[0];
    var attendMonth = $('#weektime').val().split('-')[1];
    var startDate=$('#weektime').val().split('~')[0];
    var endDate = $('#weektime').val().split('~')[1];
    var symbol = "";
    var node = $('#LeftTree').tree('getSelected')
    if (node.isleft) {
        ...
        var BusinessUnitId = node.ParentId
    }
    //var url = "/06e/x?md0=" + attendYear + "&md1=" + attendMonth + "&unitType=10&DeptId_lkid=" + unitId + "&unitId=" + unitId + "&viewMode=1";
    //var url = "/_ui/hr/attend/empshift/print?unitType=4030&unitId=" + editor.unitId + "&md0=" + editor.year + "&md1=" + editor.month;
    var url = "/attd/shifts/prtEmpDeptGroupShifts.aspx?objectTypeCode=30041&type=groupweek&md0=" + attendYear + "&md1=" + attendMonth + "&md2=0&unitId=" + unitId + "&calendar=" + 1 + "&showMemo=" + 0 + "&GroupId=" + GroupId + "&DeptId=" + DeptId + "&BusinessUnitId=" + BusinessUnitId + "&startDate=" + startDate + "&endDate=" + endDate;
    //var url = "/_ui/hr/attend/empshift/print?unitType=30041&calendar=1&unitId=" + editor.unitId + "&md0=" + editor.year + "&md1=" + editor.month;
...
function submitDeptAttendStatus(status) {
    ...
    var a = "";
    if (status == 0) {
        a = "你确定要取消提交上报  部门【" + unitName + "】  " + startDate + "年" + endDate + "周  排班吗？";
    }
    else {
        a = "你确定要提交 部门【" + unitName + "】  " + startDate + "~" + endDate + "  周排班吗？";
    }
    if (Modal.confirm(a)) {
        var Description = document.getElementById("description").value;
        var LeaveMessage = $("#LeaveMessage").val();
        var YearNum = $('#calendarHeader').html().split('-')[0]
        var url = "/apps/HrProcessor.ashx?cmd=workshift.dept.week.submit";
        jQuery.ajax({
            async: true, cache: false, dataType: "json",
            data: {
                StartDate: startDate,
...
function transferOutPeople() {
    ...
    var deptId = jQuery("#unitId").val();
    var yNum = $('#weektime').val().split('-')[0];
    var mNum = $('#weektime').val().split('-')[1];
...
function deleteSelectEmpAttendData() {
    ...
    layer.confirm('你确定要删除选择的员工排班吗？', {
        ...
        var url = "/apps/HrProcessor.ashx?cmd=workshift.dept.week.employee.delete";
        jQuery.ajax({
            async: true, cache: false, dataType: "json",
            data: {
...
function empTransfer(type) {
    ...
    if (type == 1)
        title = "调出";
    var url = "/attd/shifts/AddEmployeeToWeek.aspx?objectTypeCode=10&unitId=" + deptId + "&unitName=" + deptName + "&GroupId=" + GroupId + "&GroupName=" + GroupName + "&startDate=" + startDate + "&endDate=" + endDate + "&empName=" + empNames + "&yearNumber=" + attendYear + "&monthNumber=" + attendMonth;
    //showDialog(url, "TransferEmpGroup", title, 720, 500);
    $('.popup-mask').show()
    $('#iframe').attr('src', url).show().height(640).css('margin-top', '-320px')
}
...
function transferOutSelectEmp() {
    ...
    var unitId = node.id;
    var unitName = node.TextName;
    layer.confirm('你确定要从本周删除你选择的员工、排班数据吗？', {
        btn: ['取消', '确定'] //按钮
    }, function (index) {
        layer.close(index);
    }, function (index) {
        var url = "/apps/HrProcessor.ashx?cmd=workshift.deptgroup.employee.week.delete";
...
function sortEmpDialog() {
    ...
    var startDate = $('#weektime').val().split('~')[0];
    var endDate = $('#weektime').val().split('~')[1];
    var url1 = '/attd/shifts/sortGroupMember.aspx?objectTypeCode=10&isWeek=1&unitId=' + unitId + "&unitName=" + unitName + "&yearNumber=" + attendYear + "&monthNumber=" + attendMonth+ "&startDate=" + startDate + "&endDate=" + endDate;
...
function groupArrangeWorkshiftDialog() {
    ...
    if (selectCount < 1) {
        ...
        var unitId = node.id;
        var unitName = node.TextName;
        var yearNumber = $('#weektime').val().split('-')[0];
        var monthNumber = $('#weektime').val().split('-')[1];
        var weekNumber = "";
...
function templateArrangeWorkShiftDialog() {
    ...
    if (unitId != undefined) {
        ...
        var url = "/attd/shifts/templateArrangeWorkshift.aspx?isWeek=1&yearNumber=" + yearNumber + "&monthNumber=" + monthNumber + "&startDate=" + startDate + "&endDate=" + endDate;
...
function copyWorkshiftDialog() {
    ...
    var copyStartDate = '';
    var copyEndDate = '';
    var url = "/attd/cloneRangeShift.aspx?objectTypeCode=10&type=week&unitId=" + unitId + "&yearNumber=" + yearNumber + "&monthNumber=" + monthNumber + "&employeeIds=" + employeeIds + "&startDate=" + startDate + "&endDate=" + endDate+ "&copyStartDate=" + copyStartDate + "&copyEndDate=" + copyEndDate;
...
function exportWorkshiftDialog() {
    ...
    var url = "/attd/exportRangeShift.aspx?objectTypeCode=10&type=week&unitId=" + unitId + "&yearNumber=" + yearNumber + "&monthNumber=" + monthNumber + "&startDate=" + startDate + "&endDate=" + endDate + "&unitName=" + unitName;
    showDialog(url, "exportRangeShift", "导入排班", 1100, 580);
}
/*备注*/
...
function deletegrouppeople(groupid, employeeId, deptid) {
    if (groupid == '') {
        ...
        var startDate = $('#weektime').val().split('~')[0]
        var endDate = $('#weektime').val().split('~')[1]
        $('#iframe').attr('src', '/attd/shifts/deleteGroupRow.html?employeeId=' + employeeId + '&groupid=' + groupid + '&deptid=' + deptid + '&startDate=' + startDate + '&endDate=' + endDate + '&isWeek=1').show().height(280).css('margin-top', '-140px')
    }
}
function groupappendpeople(id, deptid) {
    ...
    var startDate = $('#weektime').val().split('~')[0]
    var endDate = $('#weektime').val().split('~')[1]
    var url = '/alert/attend/shiftgroupappendpeople.html?isWeek=1&groupid=' + id + '&deptid=' + deptid + '&startDate=' + startDate + '&endDate=' + endDate;
    $('#iframe').attr('src', url).show().height(640).css('margin-top', '-320px');
}
function upgrouppeople(groupid, employeeId, deptid, that) {
    if ($(that).parent().parent().find('td[rowspan]').length == 0) {
        ...
        ajaxMethodGetData('workshift.deptgroup.employee.week.sort', data, function (data) {
...
function downgrouppeople(groupid, employeeId, deptid, that) {
    if ($(that).parent().parent().next().find('td').length) {
...
function saveDescription(val, EmployeeId) {
    var node = $('#LeftTree').tree('getSelected');
    var unitId = node.id;
    var unitName = node.TextName;
    var startDate = $('#weektime').val().split('~')[0]
    ...
    var WeekNumber = new Date(startDate)
    var date2 = new Date(WeekNumber.getFullYear(), 0, 1)
    WeekNumber = WeekNumber.getTime()
    WeekNumber = Math.round((WeekNumber.valueOf() - date2.valueOf()) / 86400000)
    WeekNumber = Math.ceil((WeekNumber + ((date2.getDay() + 1) - 1)) / 7);

    ajaxMethodPostData('workshift.dept.week.remark', {
        unitId: unitId,
        EmployeeId: EmployeeId,
        Description: val,
        startDate: startDate,
