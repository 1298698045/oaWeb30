...
var copyData = {}
{
    ...
    attendanceEditor.prototype.unitType = 10; //10 dept 10 attendgroup
    ...
    attendanceEditor.prototype.renderFixColumns = function (d) {
        ...
        for (var j = 0; j < groups.length; j++) {
            for (var i = 0; i < groups[j].data.length; i++) {
                ...
                html2 += "<td class=\"nameCol1\" nowrap='nowrap' style=\"vertical-align:middle;\">" + emp.Name + "</td>";
                html2 += "<td class=\"nameCol1\" style=\"vertical-align:middle;\">" + emp.EmployeeNo + "</td>";
                //html2 += "<td nowrap=\"nowrap\">" + (emp.NurseLevel||'') + "</td>";
    ...
    attendanceEditor.prototype.rendertable = function () {
        ...
        prea += "&startDate=" + $('#weektime').val().split('~')[0];
        prea += "&endDate=" + $('#weektime').val().split('~')[1];
        //prea += '&pagesize=' + $('#pagesizecheck').val()
        //prea += '&page=' + $('#pageto').val()
        var that = this
        this.loadData("workshift.dept.employee.week.search", prea, function (data) {
...
function printAttenanceCalendar() {
    ...
    var symbol = "";
    var md0 = $('#weektime').val().split('~')[0]
    var md1 = $('#weektime').val().split('~')[1]
   ...
   // var url = "/_ui/hr/attend/empshift/print?objectTypeCode=10&md0=" + attendYear + "&md1=" + attendMonth + "&md2=0&unitId=" + unitId + "&calendar=" + 1 + "&showMemo=" + 0;
    //var url = "/_ui/hr/attend/empshift/print?unitType=10&calendar=1&unitId=" + editor.unitId + "&md0=" + editor.year + "&md1=" + editor.month;
    var url = "/attd/shifts/prtEmpDeptGroupShifts.aspx?unitType=10&DeptId=" + unitId + "&md0=" + attendYear + "&md1=" + attendMonth + "&type=deptweek" + "&startDate=" + md0 + "&endDate=" + md1;
...
function refreshAttendCell(data, cellId, copydata) {
    ...
    if (data.Shifts) {
        for (var j = 0; j < data.Shifts.length; j++) {
            ...
            var BusinessUnitIdName = row.BusinessUnitIdName
            var CreatedByName = row.CreatedByName
            var GroupIdName = row.GroupIdName
            var CheckTimes = row.CheckTimes;
            var WorkHours = row.WorkHours;
            var Name = row.Name;
            ...
            if (canEdit == false) {
                ...
                tempCell = "<div title='报送部门/组：" + BusinessUnitIdName + (BusinessUnitIdName && GroupIdName ? "/" : '') + GroupIdName + "，报送人:" + CreatedByName + "。班次名称：" + Name + "，上下班次数：" + CheckTimes + "，应出勤时长：" + WorkHours + "，第一次上班：" + StartTime1 + "，第一次下班：" + EndTime1 + (CheckTimes==2 ? "，第二次上班：" + StartTime2 : '') + (CheckTimes==2 ? "，第二次下班：" + EndTime2 : '') + (CheckTimes==3 ? "，第三次上班：" + StartTime3 : '') + (CheckTimes==3 ? "，第三次下班：" + EndTime3 : '')  + "。'class=\"qingjia1\" style='cursor:pointer;vertical-align:middle;cursor:pointer;text-align:center;margin:3px;" + style + "'";
                tempCell += " onclick=\"delAttendEmpDay(this,'" + row.EmpWorkShiftId + "','" + cellId + "','" + row.EmployeeId + "','" + row.WorkDay + "')\"";
...
function submitDeptAttendStatus(status) {
    ...
    var a = "";
    if (status == 0) {
        a = "你确定要取消提交上报  部门【" + unitName + "】  " + startDate + "年" + endDate + "  排班吗？";
    }
    else {
        a = "你确定要提交上报 部门【" + unitName + "】  " + startDate + "~" + endDate + "  排班吗？";
    }
    if (Modal.confirm(a)) {
        var Description = document.getElementById("description").value;
        var LeaveMessage = $("#LeaveMessage").val();
        var YearNum = $('#weektime').val().split('-')[0]
...
function empTransfer(type) {
    ...
    if (type == 1)
        title = "调出";
    var url = "/attd/shifts/AddEmployeeToWeek.aspx?objectTypeCode=10&type=dept&unitId=" + unitId + "&unitName=" + unitName + "&yearNumber=" + attendYear + "&monthNumber=" + attendMonth + "&empName=" + empNames + "&startDate=" + startDate + "&endDate=" + endDate;
...
function batchArrangeWorkshiftDialog() {
    ...
    if (selectCount < 1) {
        ...
        var monthNumber = ''
        var weekNumber = "";
        var sDate = $('#weektime').val().split('~')[0];// document.getElementById("startDate").value;
        var eDate = $('#weektime').val().split('~')[1];//document.getElementById("endDate").value;
        var url = "/hr/attend/CreateEmpWorkShift.aspx?startDate=" + sDate + "&endDate=" + eDate + "&year=" + yearNumber + "&month=" + monthNumber + "&md2=" + weekNumber + "&objectTypeCode=10&unitId=" + unitId + "&groupId=" + unitId + "&retURL=/_ui/core/hr/dept/ui/WeekshiftPage";
...
function templateArrangeWorkShiftDialog() {
    ...
    if (unitId != undefined) {
        ...
        var url = "/attd/shifts/templateArrangeWorkshift.aspx?isWeek=1yearNumber=" + yearNumber + "&monthNumber=" + monthNumber + "&startDate=" + startDate + "&endDate=" + endDate;
...
function remarkDialog() {
    ...
    var attendYear = $('#weektime').val().split('-')[0]
    var attendMonth = $('#weektime').val().split('-')[1]
    var url1 = '/attd/shifts/RemarkWorkshift.aspx?type=week&objectTypeCode=10&unitId=' + unitId + "&unitName=" + unitName + "&yearNumber=" + attendYear + "&monthNumber=" + attendMonth;
