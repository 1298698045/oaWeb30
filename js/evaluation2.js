...
function appendAddScore(AddScore, addTotalPoints, length) {
    ...
    for (var i = 0; i < AddScore.length; i++) {
        ...
        var id = AddScore[i].ValueId
        html += '<tr>'
        html += '<td>加分</td><td>' + xname + '</td><td>' + addpoint + '</td><td>' + remark + '</td><td style="width:150px;"><span class="addscorebtn" onclick=\'addscoreedit("' + id + '","' + xname + '")\'>编辑</span>&nbsp;&nbsp;&nbsp;&nbsp;<span class="addscorebtn" onclick=\'addscoredel(30235,"' + id + '")\'>删除</span></td>'
        html += '</tr>'
    }
    html += '<tr>'
    html += '<td>合计</td><td></td><td>' + addTotalPoints + '</td><td></td><td></td>'
    html += '</tr>'
    $('#Scoretable').append(html)
}
function appendMinuScore(MinuScore, minTotalPoints, length) {
    ...
    for (var i = 0; i < MinuScore.length; i++) {
        ...
        //html += '</tr>'
        html += '<tr>'
        html += '<td>减分</td><td>' + xname + '</td><td>' + minupoint + '</td><td>' + remark + '</td><td></td>'
        html += '</tr>'
        html += '<tr>'
        html += '<td>合计</td><td></td><td>' + minTotalPoints + '</td><td></td><td></td>'
        html += '</tr>'
    }
    $('#Scoretable').append(html)
...
function addscore() {
    $('.popup-mask').show()
    $("#iframe").attr("src", "/alert/evaluation/eval_scoreadd.html?EmployeeId=" + getQueryString('EmployeeId')).show().height(620).css({ "left": "20%", "background-color": "white", "width": "60%", "max-height": "640px" });
}
function addscoreedit(id,Name) {
    $('.popup-mask').show()
    $("#iframe").attr("src", "/alert/evaluation/eval_scoreadd.html?EmployeeId=" + getQueryString('EmployeeId')+"&edit=1&id=" + id+"&Name=" + Name).show().height(620).css({ "left": "20%", "background-color": "white", "width": "60%", "max-height": "640px" });
}
function addscoredel(objTypeCode, id) {
    $('.popup-mask').show()
    $('#iframe').attr('src', '/alert/delete_ie8.html?objTypeCode=' + objTypeCode + '&id=' + id).show().height(280).css({ "left": "20%", "background-color": "white", "width": "60%", "max-height": "280px" });
}
function refreshScoretable(){
    ...
    $.ajax({
        ...
        type: 'post',
        data: {
            EvaluationId: getQueryString('evaluationId'),
            EmployeeId: getQueryString('employeeId'),
            EvalTypeCode: getQueryString('evalTypeCode'),
            EvaluationFormId: getQueryString('evaluationFormId')
        },
        dataType: 'json',
        async: false,
        success: function (data) {
            var formData = data.listData;
            if (formData.length > 0) {
                $('#Scoretable').append('<tr><th colspan="6" style="position: relative;">加减分<span class="scorebtn" onclick="addscore()" id="scorebtn">添加加分<span></th></tr>')
                var html = '<tr><th>加减分类型</th><th>加减分名称</th><th>加减分分数</th><th>备注</th><th>操作</th></tt>'
                $('#Scoretable').append(html);

                that.appendAddScore(formData[0].AddScore, formData[0].AddTotalPoints, '')
                that.appendMinuScore(formData[0].MinuScore, formData[0].MinuTotalPoints, '')

                if (formData[0].TotalPoints >= 0) {
                    $('#TotalPoints').text(formData[0].TotalPoints);
                    $('#ResultLevel').text(formData[0].ResultLevel);
                }
            }
        }
    });
