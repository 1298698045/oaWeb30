...
function getQueryStringEmpty(name) {
    var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)", "i");
    var r = location.search.substr(1).match(reg);
...
function ajaxMethodData(method, d, callback) {
    var url = rootUrl + "&cmd=" + method;
    //url += queryString;
    jQuery.ajax({
        async: false, cache: false, dataType: "json",
...
function calculateLeaveDays(leaveType, startTime, leavedays, callbak) {
    ...
    var url = "/apps/HrProcessor.ashx?cmd=attendance.leavetypeaccount.check&leavetype=" + leaveType + "&startTime=" + startTime + "&leavedays=" + leavedays;
    jQuery.ajax({
        async: false, cache: false, dataType: "json",
        error: function (request, textStatus, errorThrown) {
            errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
function getDepartments(renderCallback, dformat) {
    ...
    jQuery.ajax({
        async: true, cache: false, dataType: dataT, error: function (request, textStatus, errorThrown) { errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;"); messageOptions_handleAjaxErrors([errorThrown ? errorThrown : textStatus]); },
        success: function (data, textStatus) {
            // debugger;
            renderCallback(data);
        },
...
function getRoleSystemUsers(renderCallback, roleId) {
    ...
    jQuery.ajax({
        async: true, cache: true, dataType: "json", error: function (request, textStatus, errorThrown) { errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;"); messageOptions_handleAjaxErrors([errorThrown ? errorThrown : textStatus]); },
        success: function (data, textStatus) {
            // debugger;
            renderCallback(data);
        },
...
function searchAllDepts(renderCallback, srch) {
    //var roleId = jQuery("#id").val();
    //if (userId == undefined) return;
    var url = "/SecurityProcessor.ashx?cmd=searchdepts&search=" + encodeURIComponent(srch);
    jQuery.ajax({
        async: true, cache: false, dataType: "json", error: function (request, textStatus, errorThrown) { errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;"); messageOptions_handleAjaxErrors([errorThrown ? errorThrown : textStatus]); },
        success: function (data, textStatus) {
            // debugger;
...
function searchAllEmployee(renderCallback, srch) {
    //var roleId = jQuery("#id").val();
    //if (userId == undefined) return;
    var url = "/SecurityProcessor.ashx?cmd=pick.employee.search&search=" + encodeURIComponent(srch);
    jQuery.ajax({
        async: true, cache: false, dataType: "json", error: function (request, textStatus, errorThrown) { errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;"); messageOptions_handleAjaxErrors([errorThrown ? errorThrown : textStatus]); },
        success: function (data, textStatus) {
            // debugger;
...
function getUnreadEmailQty(renderCallback) {
    var url = "/apps/CommandProcessor.ashx?cmd=email.stat";
...
function renderMailbox(d) {
    ...
    for (var i = 0; i < d.data.length; i++) {
        ...
        {
            //var a = item.toUserNames.replace(/,/ig, '');
            // html += '<span class="names c">' + item.toUserNames + '</span></td>';
            html += '<span class="names">' + item.toUserNames + '</span></td>';
    ...
    //        for (var i = 0; i < $("#mails_body input").length; i++) {
    //            $("#mails_body input").eq(i).attr('checked', true);
    //        }
    //    }
    //    else {
    //        for (var i = 0; i < $("#mails_body input").length; i++) {
    //            console.log($("#chidItems .pretty").eq(i).children('input'));
    //            $("#mails_body input").eq(i).attr('checked', false);
    //        }
    //    }
    //})
}

function renderMailboxNewUI(d) {
    var html = "";
    for (var i = 0; i < d.data.length; i++) {
        var item = d.data[i];
        ...
        html += '<td class="dataCell">';
        if (item.mailTag == "draft") {
            html += '<a title="" class="actionLink" href="/email/WriteEmail.aspx?t=InboxEmail&ltags=' + item.mailTag + '&id=' + item.emailId + '">';
        } else {

            html += '<a title="" class="actionLink" href="/email/CheckMail.aspx?t=InboxEmail&ltags=' + item.mailTag + '&id=' + item.emailId + '">';
        }
...
function checkResourceFree(resourceId, startTime, endTime, meetingid, renderCallback) {

    var url = "/apps/CommandProcessor.ashx?cmd=checkresourcefree&resourceId=" + resourceId + "&startTime=" + startTime + "&endTime=" + endTime + "&meetingid=" + meetingid;
    jQuery.ajax({
...
function synMutipleCheckboxValue(cId, vValue) {
    var selValue = "";
    var sort = 0;
    jQuery("input[name='" + cId + "_m']").each(function (index, ele) {
        var isSelect = ele.checked;// jQuery(this).attr("checked");
        if (isSelect) {
...
function sortTemplateField(templateid, id, sortOrder, direct) {
    var url = "/apps/CommandProcessor.ashx?cmd=sorttemplatefield&templateid=" + templateid + "&fieldid=" + id + "&sortnumber=" + sortOrder + "&direct=" + direct;
    jQuery.ajax({
        async: true, cache: false, dataType: "json",
        error: function (request, textStatus, errorThrown) {
            errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
function saveMeetingItem(id, desc) {
    //debugger;
    var url = "/apps/CommandProcessor.ashx?cmd=savemeetingitem";
    jQuery.ajax({
        async: true, cache: false, dataType: "json",
        data: {
            id: id,
...
function setVolunteerActivityAppoint(elmid, date) {
    var url = "/apps/CommandProcessor.ashx?cmd=volunteerappoint&date=" + date;
    jQuery.ajax({
        async: true, cache: false, dataType: "json",
        error: function (request, textStatus, errorThrown) {
            errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
/*值班*/
function getGroupAndMembers(callback, isPublic) {
    var url = "/apps/CommandProcessor.ashx?cmd=sys.group.getlist&isPublic=" + isPublic;
    jQuery.ajax({
        async: true, cache: false, dataType: "json",
        error: function (request, textStatus, errorThrown) {
            errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
/*值班排班*/
function setDutyshiftAppoint(eleId, day, callback) {
    var userId = document.getElementById("currentEmp").value;
    if (userId == "") {
        alert("请先选择排班人！");
        return;
    }
    var categoryid = document.getElementById("cateid").value;
    var currentEmpName = document.getElementById("currentEmpName").value;
    var shifttype = jQuery("#" + eleId).attr("data-type");
    //alert(shifttype);
    var url = "/apps/HrProcessor.ashx?cmd=attend.dutyshift.set&date=" + day + "&userid=" + userId + "&shifttype=" + shifttype;
    url += "&categoryid=" + categoryid + "&name=" + encodeURIComponent(currentEmpName);
    //alert(url);
    jQuery.ajax({
        async: true, cache: false, dataType: "json",
        error: function (request, textStatus, errorThrown) {
        ...
        success: function (data, textStatus) {
            ...
            //window.location.reload();
            //renderCallback(data);
            var empName = document.getElementById("currentEmpName").value;
            jQuery("#" + eleId).html(empName);
            //jQuery("#" + eleId).attr("class", "");
            //jQuery("#" + eleId).removeClass("calendarOff");
            //jQuery("#" + eleId).attr("class", "calendarOn");
            document.getElementById(eleId).className = "calendarOn";
            //jQuery("#" + eleId).removeAttr("onclick");
            jQuery("#" + eleId).attr("onclick", "delDutyshiftAppoint('" + eleId + "','" + userId + + "','" + day + "')");
        },
        type: "POST",
        url: url
    });
}
function delDutyshiftAppoint(eleId, userId, day, callback) {
    //var userId = document.getElementById("currentEmp").value;
    //var currentEmpName = document.getElementById("currentEmpName").value;
    //if (userId == "") return;
    var url = "/apps/HrProcessor.ashx?cmd=attend.dutyshift.delete&date=" + day + "&userid=" + userId;
    //alert(url);
    jQuery.ajax({
        async: true, cache: false, dataType: "json",
        error: function (request, textStatus, errorThrown) {
        ...
        success: function (data, textStatus) {
            ...
            //var empName = document.getElementById("currentEmpName").value;
            jQuery("#" + eleId).html("");
            document.getElementById(eleId).className = "calendarOff";
            //jQuery("#" + eleId).attr("class", "calendarOff");
            jQuery("#" + eleId).attr("onclick", "setDutyshiftAppoint('" + eleId + "','" + day + "')");
        },
        type: "POST",
        url: url
    });
}
function deleteMediaImg(imgId,contentid, callback) {
    var url = "/apps/CommandProcessor.ashx?cmd=media.delete&id=" + imgId+"&contentid="+contentid;
    jQuery.ajax({
        async: true, cache: false, dataType: "json",
        error: function (request, textStatus, errorThrown) {
            errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
function deleterRelatedAttachment(attachId, callback) {
    var url = "/apps/CommandProcessor.ashx?cmd=attach.delete&id=" + attachId;
    jQuery.ajax({
        async: true, cache: false, dataType: "json",
        error: function (request, textStatus, errorThrown) {
            errorThrown = String(errorThrown).replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/'/g, "&#39;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
...
jack:
2021-09-17
processInstanceId:实例名称，是实体记录的ID
ProcessId:流程定义ID
name: 实例名称， 如 “合同审计名称”
*/
function processEntityIntanceSubmit(processInstanceId,processId, name)
{
    var d = {
        "params": {
            "recordRep": {
    ...
    ajaxMethodPostData('process.entity.instance.submit', { message: message }, function (data) {
