            ...
            for (var h = 0; h < days; h++) {
                ...
                if (this.viewMode == 1) {
                    ...
                    for (var j = 0; j < row.Shifts.length; j++) {
                        ...
                        if (canEdit == false) {
                            tempCell = "<div title='报送部门:" + BusinessUnitIdName + " 报送人:" + CreatedByName + "' class=\"qingjia1 cannotEdit\" style='cursor:pointer;vertical-align:middle;cursor:pointer;text-align:center;margin:3px;" + style + "'";;
                        } else {
                            tempCell = "<div title='报送部门:" + BusinessUnitIdName + " 报送人:" + CreatedByName + "' class=\"qingjia1\" style='cursor:pointer;vertical-align:middle;cursor:pointer;text-align:center;margin:3px;" + style + "'";
                            if (isEditable)
                                tempCell += " onclick=\"delAttendEmpDay('" + shift.EmpWorkShiftId + "','" + cellId + "','" + emp.EmployeeId + "','" + row.AttendDate + "')\"";
                        }

                        tempCell += " id='c_" + emp.EmployeeId + "_" + row.AttendDate + "_" + shift.EmpWorkShiftId + "' data-status='{2}'>" + shift.WorkShiftIdName + "</div>";
    ...
    if (data.Shifts) {
        for (var j = 0; j < data.Shifts.length; j++) {
            ...
            if (canEdit == false) {
                ...
                tempCell = "<div title='报送部门:" + BusinessUnitIdName + " 报送人:" + CreatedByName + "' class=\"qingjia1\" style='cursor:pointer;vertical-align:middle;cursor:pointer;text-align:center;margin:3px;" + style + "'";
                tempCell += " onclick=\"delAttendEmpDay('" + shift.EmpWorkShiftId + "','" + cellId + "','" + shift.EmployeeId + "','" + row.AttendDate + "')\"";
            }

            tempCell += " id='c_" + row.EmployeeId + "_" + row.AttendDate + "_" + row.EmpWorkShiftId + "' data-status='{2}'>" + row.WorkShiftIdName + "</div>";
...
function deleteSelectEmpAttendData() {
    ...
    var unitId = jQuery("#unitId").val();
    if (Modal.confirm("你确定要删除你选择的员工排班吗？")) {
...
function exchangeEmployeeShiftDialog() {
    ...
    var url = "/attd/shifts/exchangeShift.aspx?objectTypeCode=10&unitId=" + unitId;
...
function templateArrangeWorkShiftDialog() {
    ...
    if (unitId != undefined) {
        ...
        var url = "/attd/shifts/templateArrangeWorkshift.aspx?yearNumber=" + yearNumber + "&monthNumber=" + monthNumber;
        url += "&objectTypeCode=10&unitId=" + unitId;
        url += "&employeeIdNames=" + employeeIdNames + "&employeeIds=" + employeeIds;

        showDialog(url, "arrangeGroupWorkshift", "模板排班", 800, 485); return false;
    }
}
/*复制排班*/
function copyWorkshiftDialog() {
    ...
    var yearNumber = jQuery("#yearNumber").val();
    var monthNumber = jQuery("#monthNumber").val();
    var url = "/attd/cloneRangeShift.aspx?objectTypeCode=10&unitId=" + unitId + "&yearNumber=" + yearNumber + "&monthNumber=" + monthNumber + "&employeeIds=" + employeeIds
