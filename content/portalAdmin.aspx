...
onLoadSelector see Bug W-1599971 */ }
                                ...
                                <div class="miss-header" style="background-color:white;border-bottom:1px solid #dedede">
                                    <div class="header-left-logo">
                                        <img src="/template/hearder/logo.png"/>
                                    </div>
                                    <div class="header-left-title">
                                        <p class="header-left-title-name">门户</p>
                                        <p class="header-left-title-listname selectinput"><span>门户设置</span></p>
                                    </div>
        
                                    <div class="header-right-buttonbtn">
                                        <div class="editbtns" id="pageedit" style="display:block;">
                                              <div class="newpage">新建门户</div>
                                              <div class="editpage">编辑门户</div>
                                              <div class="deletepage">删除门户</div>
                                              <div class="privilegepage">设置权限</div>
                                              <div class="sortpage">门户排序</div>
                                              <div class="newblock">新建区块</div>
                                              <div class="sortblock">区块排序</div>
                                         </div>
                                    </div>
                                    <div class="clear"></div>
                                </div>
    ...
    <script>
        ...
        $('.ct').height($(window).height() - 190)
        $('.left-table').height($(window).height() - 190)
        getpage()
        function getpage() {
            ajaxMethodGetData('page.getlist', {scope:1}, function (data) {
        ...
        function getlist(that) {
            ...
            ajaxMethodGetData('page.block.content.getlist', { blockId: blockId }, function (data) {
                for (var i = 0; i < data.contents.length; i++) {
                    ...
                    $(that).find('ul').append('<li title="' + res.Title + '"><a target="' + res.Title + '" href="/Portal/viewArticle.aspx?id=' + res.ContentId + '" class="item-name">' + res.Title + '</a><span class="item-time">' + res.CreatedOn.split('T')[0] + '</span></li>')
                }
            })
        }
        ...
        //<div class="deletepage">删除门户</div>
        //<div class="privilegepage">设置权限</div>
        //<div class="privilegepage">门户排序</div>
        $('.newpage').click(function () {
            $('#iframe').attr('src', '/layouteditor/add_layout_ie8.html?entityName=ContentPage').height(640).css('margin-top', '-330px').show()
            $('.popup-mask').show()
        })
        $('.editpage').click(function () {
            var id = $('.left-table .active').attr('pageid')
            $('#iframe').attr('src', '/layouteditor/add_layout_ie8.html?entityName=ContentPage&id='+id).height(640).css('margin-top', '-330px').show()
            $('.popup-mask').show()
        })
        $('.sortpage').click(function () {
            var id = $('.left-table .active').attr('pageid')
            $('#iframe').attr('src', '/alert/content/sort.html?type=sortpage').height(460).css('margin-top', '-230px').show()
            $('.popup-mask').show()
        })
        $('.deletepage').click(function () {
            var id = $('.left-table .active').attr('pageid')
            if (confirm('确定要删除吗？')) {
                ajaxMethodGetData('entity.delete', { id: id, ObjTypeCode: 100211 }, function (data) {
                    window.location.reload()
                })
            }
        })
        $('.privilegepage').click(function () {
            var id = $('.left-table .active').attr('pageid')
            var name = $('.left-table .active').find('span').html()
            $('#iframe').attr('src', '/alert/privilege/privilegeSave.html?type=sortpage&id=' + id + '&name='+name).height(500).css('margin-top', '-230px').show()
            $('.popup-mask').show()
        })
    </script>
</body>
