...
<head>
    ...
    <style type="text/css">
        ...
        .search-cont.header-right-buttonbtn {
            margin-bottom:10px;
        }
        .morebtn .selectlist li {
            width:150px;
                    ...
                    <td class="oRight" id="bodyCell">
                        ...
                        <div class="miss-header">
                            ...
                            <div class="header-right-buttonbtn">
                                ...
                                <div class="buttonbtn-group">
                                    <div class="buttonbtn addsubject">议题上会安排</div>
                                </div>
                                <div class="buttonbtn-group">
                                    ...
                                    <div class="buttonbtn selectbuttonbtn morebtn">
                                        <img class="morebtn-img" src="/img/cal/20.png" alt="" />
                                        <div class="selectlist" style="display: none;">
                                            <ul>
                                                ...
                                                <li class="printpeople3">打印 请假人员</li>
                                                <li class="printpeople4">打印 未请假人员</li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                                           ...
                                           <div class="search-group" 
                                                ...
                                                <div class="selectlist selectlistcheck">
                                                    <ul>
                                                        ...
                                                        <li search="1"><img src="/img/cal/23.check_2(16x16).png" /><span>已接收</span></li>
...
<%--                                    <div class="search-group commentsearch" 
    ...
    <div class="file-preview" style="display: none;">
        ...
        <iframe frameborder="0" src="" onload="shareIframeImg(this)" style="height: 89%;"></iframe>
    </div>
    ...
    <script>
        ...
        $('.mtg-delete').click(function () {
            ...
            $('#iframe').attr('src', '/alert/delete_ie8.html?id=' + id + '&objTypeCode=5000').show().height(280).css('margin-top', '-140px')
            $('.popup-mask').show()
        })
        //邀请参会者
        ...
        $('.finishbtn').click(function () {
            $('.popup-mask').show()
            $('#iframe').attr('src', '/alert/item_finish_ie8.html').show().height(280).css('margin-top', '-200px')
        })
        //提醒
        $('.megtixing').click(function () {
            ...
            $('.popup-mask').show()
            $('#iframe').attr('src', '/alert/add_tixing_ie8.html?id=' + metid + "&objectTypeCode=" + objectTypeCode).show().height(520).css('margin-top', '-300px')
        ...
        $('.megedit').click(function () {
            $('.popup-mask').show()
            //$('#iframe').attr('src', '/alert/meeting/add_meeting_ie8.html?id=' + metid).show().height(620).css('margin-top','-300px')
            $('#iframe').attr('src', '/alert/meeting/newmeeting.html?entityName=MeetingRec&objectTypeCode=5000&id=' + metid).show().height(620).css('margin-top', '-300px')
        })
        //新建议题
        $('.megitem').click(function () {
            $('.popup-mask').show()
            $('#iframe').attr('src', '/alert/meeting/add_meetingitem_ie8.html').show().height(635).css('margin-top', '-300px')
        })
        function megitem(id) {
            $('.popup-mask').show()
            $('#iframe').attr('src', '/alert/meeting/add_meetingitem_ie8.html?id=' + id).show().height(635).css('margin-top', '-300px')
        }
        //新建任务
        $('.megcal').click(function () {
            $('.popup-mask').show()
            $('#iframe').attr('src', '/alert/meeting/add_meetingcal_ie8.html').show().height(620).css('margin-top', '-300px')
        })
        function megcal(id) {
            $('.popup-mask').show()
            $('#iframe').attr('src', '/alert/meeting/add_meetingcal_ie8.html?id=' + id).show().height(620).css('margin-top', '-300px')
        }
        //打印人员
        ...
        $('.printpeople3').click(function () {
            //请假
            window.open('/apps/meetings/printMeetingPeoplelst.aspx?status=3' + '&id=' + metid, "_blank", "scrollbars=yes,resizable=1,modal=false,alwaysRaised=yes")
        })
        $('.printpeople4').click(function () {
            //未请假
            window.open('/apps/meetings/printMeetingPeoplelst.aspx?status=4' + '&id=' + metid, "_blank", "scrollbars=yes,resizable=1,modal=false,alwaysRaised=yes")
        })
        //签到
        ...
        function itemdelete(id,objTypeCode) {
            $('#iframe').attr('src', '/alert/delete_ie8.html?id=' + id + '&objTypeCode=' + objTypeCode).height(280).css('margin-top', '-140px');
            $('#iframe').show();
            $('.popup-mask').show()
        }
        $('.task-delete').click(function () {
            $('#iframe').attr('src', '/alert/delete_ie8.html?id=').height(280).css('margin-top', '-140px');
            $('.popup-mask').show()
            $('#iframe').show();
        })
        //获取详情
        ...
        function getdata() {
            ...
            function callback(data){
                ...
                var InviteQty = mtgview.InviteQty
                var CreatedUserId = mtgview.CreatedBy.Id
                ...
                $('.contract-message ul').eq(1).find('li').eq(0).html(ScheduledStart)
                $('.contract-message ul').eq(1).find('li').eq(1).html(ScheduledEnd)
                $('.contract-message ul').eq(1).find('li').eq(2).html(Location || RoomIdName)
                $('.contract-message ul').eq(1).find('li').eq(3).html(InviteQty)
                $('.contract-message ul').eq(1).find('li').eq(4).html(StatusCode)
                $('.Name').html(Name)
        ...
        function getsummary() {
            ...
            ajaxMethodPostData('meeting.summary.get', d, function (data) {
                $('.summary #tinymce').html(data.data)
            })
        }
        ...
        $('.newfileclass').click(function () {
            $('.selectlist').hide()
            $('#iframe').attr('src', '/alert/cntrt/new_fileclass_ie8.html');
            $('#changeDiv').show();
            $('#iframe').show();
            return false
        })
        ...
        function gettask() {
            ...
            function callback(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    var OwningBusinessUnitName = res.OwningBusinessUnitName
                    var ActualStart = res.ActualStart
                    var ActualEnd = res.ActualEnd
                    var ActualDurationMinutes = res.ActualDurationMinutes
                  ...
                  + '</div>'
                                                + '<div class="task-item-body">'
                                                    + '<div><span>计划开始时间:</span><span>' + ScheduledStart + '</span></div>'
                                                    + '<div><span>计划截止时间:</span><span>' + ScheduledEnd + '</span></div>'
                                                    + '<div><span>被分配人:</span><span>' + OwningUserName + '</span></div>'
                                                    + '<div><span>被分配人所在部门:</span><span>' + OwningBusinessUnitName + '</span></div>'
                                                    + '<div><span>状态:</span><span>' + ScheduledEnd + '</span></div>'
                                                    + '<div><span>实际开始时间:</span><span>' + ActualStart + '</span></div>'
                                                    + '<div><span>实际结束时间:</span><span>' + ActualEnd + '</span></div>'
                                                    + '<div><span>耗时:</span><span>' + ActualDurationMinutes + '</span></div>'
                                                + '</div>'
                                            + '</div>')
                }
                selectbuttonbtn()
        ...
        $('.mtg-status').click(function () {
            $('#iframe').attr('src', '/alert/status_edit_ie8.html?').height(480).css('margin-top', '-230px');
            $('#iframe').show()
            $('.popup-mask').show()
        })
    ...
    <script>
        ...
        $('.addsubject').click(function () {
            $('#iframe').show().attr('src', '/alert/lookupiframe.html?objtypecode=5001&title=议题').height(640).css('margin-top', '-320px')
            $('.popup-mask').show()
        })
        function lookupPick(a,b,c,d,id,name) {
            console.log(id)
            console.log(name)
            var message = formatmessagesave({ MeetingId: { Id: getQueryString('id') } }, 5001, id)
            ajaxMethodGetData('entity.saverecord', { message: message }, function (data) {
                success2('保存成功')
                getitem()
            })
        }
