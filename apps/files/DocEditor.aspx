  ...
  '</customUI>';
    ...
    <script type="text/javascript">
        ...
        function WebRefuseEdit() {
            var mFlag = window.confirm("您确定要拒绝对正文所做的所有修订吗？如果确定，所有的修订将全部无效！");
            if (mFlag) {
                //Form1提示报错，将替换成webform  --许春尧修改
        ...
        <tr>
            <td align="right" valign="middle" height="30" width="100%" style="padding-right: 11px"
                ...
                <select name="dropOpenType" id="dropOpenType" onchange="linkDisabled(0)" style="height:25px;">
	                <option selected="selected" value="0">Word文档</option>
	                <option value="1">Excel文档</option>
	                <!--<option value="3">PDF文档</option>-->
                </select>
                ...
                <input type="button" class="btn_delete" value="文字批注" onclick='webform.WebOffice.ShowType = 1;' />
                <input type=button class="btn_revision" value="显示痕迹" <%=mDisabled%>  onClick="ShowRevision(true)">
                <input type=button class="btn_revision" value="隐藏痕迹" <%=mDisabled%> onClick="ShowRevision(false)">
                <input type=button class="btn_revision" value="获取痕迹" <%=mDisabled%>  onClick="WebGetRevisions()">
                <!--<input type=button class="btn_revision" value="清除痕迹" <%=mDisabled%>  onClick="WebAcceptAllRevisions()">-->
