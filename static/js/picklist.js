...
picklist.prototype.onLoad = function () {
    if (!this.loaded) {
        ...
        a && (this.container = a.parentNode, a = null, this.isDependent() && (a = getElementByIdCS(this.controller_id)), a ? (a.dependents ? a.dependents.push(this) : (a.dependents = [this], a.getControllingValue = picklist.getControllingValue, "checkbox" == a.type ? (a.dplOldHandler = a.onclick, a.onclick = picklist.onControllerChange) : (a.dplOldHandler =
            a.onchange, a.onchange = picklist.onControllerChange)), this.generate(a.getControllingValue())) : this.generate(null))
    }
};
...
multiPicklist.prototype.displaySelectElement = function (a, b, c, d, f, e) {
    ...
    l && (k.push('\x3coptgroup style\x3d"font-size:9px;text-decoration:none;" label\x3d"' + h + '"\x3e'), isFirefox && k.push("\x3c/optgroup\x3e"));
    e = 0;
    if (this.firstRound)
        for (var m in c) d && c[m] != picklistNAMarker && "function" != typeof c[m] ? (k.push('\x3coption value\x3d"' + e++ + '"\x3e'), k.push(c[m])) : e++;
    m = 0;
    for (c = e - 1; m < this.values.length; m +=
        2)
        if (!this.isDependent() || a.testBit(m / 2)) c++, void 0 != b[this.values[m]] == d && (k.push('\x3coption value\x3d"' + c + '"\x3e'), k.push(this.values[m + 1]));
