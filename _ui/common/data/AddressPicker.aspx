<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AddressPicker.aspx.cs" Inherits="WebClient._ui.common.data.AddressPicker" %>
...
<body class="hasMotif homeTab popup popupTab brandNoBgrImg EmailCCBccLookup">
    ...
    <script type="text/javascript">
        function switchType(sel) {
            if (sel.value == "S") {
                var str1 = jQuery("#searchValue_sharing_search").val();
                deptChange(str1);
        ...
        function doSearch()
        {
            ...
            //if (sel.value == "S") {
                var str1 = jQuery("#searchValue_sharing_search").val();
            //deptChange(str1);
            getAddressLists(fillUserList, "O", "", str1);
                //getDepartments(fillDeptList, 2);
            //    jQuery("#deptContainer").show();
            //}
            //else {
        ...
        function deptChange(d) {
            ...
            //if (t.value == "U")
            {
               // getDepartmentUsers(fillUserList, d)
                getAddressLists(fillUserList, type, d, "");
            }
        }
        function fillUserList(data) {
            ...
            if (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var item = data[i];
                   // sele.add(new Option(item.name, type + ':' + item.id));
        ...
        <div>
            <label for="sharing_search">
                搜索：</label>&nbsp;<select title="小组类型" onchange="switchType(this)" name="sharing_search"
                    id="sharing_search">
                     <option value="O">单位同事</option>
                     <option value="S">公用通讯录</option>
                     <option value="P">我的通讯录</option>
                </select>
            &nbsp;&nbsp; <label for="searchValue_sharing_search">
                属于：</label>&nbsp;<input type="text" size="20" onpropertychange="duel_select_0Var.handlePropertyChange();"
                    ...
                    maxlength="80" id="searchValue_sharing_search">&nbsp;<input type="button" title="查找"
                        onclick="doSearch();" name="go_duel_select_0" id="go_duel_select_0Var"
                                                ...
                                                <td class="selectCell">
                                                    <div class="selectTitle">
                                                        <label for="duel_select_0">
                                                            可用</label></div>
                                                    <select size="14" name="duel_select_0" multiple="multiple" id="duel_select_0" ondblclick="selectItems()">
                                                    </select>
                                                </td>
                                                ...
                                                <td class="selectCell">
                                                    <div class="selectTitle">
                                                        <label for="duel_select_1">
                                                            已选者：</label></div>
                                                    <select size="14" name="duel_select_1" multiple="multiple" id="duel_select_1" ondblclick="removeItems()">
                                                    </select>
                                                </td>
                                            </tr>
                                        </tbody>
    ...
    <script type="text/javascript">
        // filterType("A");
        var duel_select_0Var = new SelectFilterElement("sharing_search", "O", "duel_select_0", "", "duel_select_1", 100, true, null);
