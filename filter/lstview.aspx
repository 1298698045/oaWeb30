...
<body onunload="if(this.bodyOnUnload)bodyOnUnload();" onbeforeunload="if(this.bodyOnBeforeUnload){var s=bodyOnBeforeUnload();if(s)return s;}" onload="if(this.bodyOnLoad)bodyOnLoad();" class="hasMotif contactTab  overviewPage sfdcBody brandQuaternaryBgr   ext-gecko ext-gecko3" onfocus="if(this.bodyOnFocus)bodyOnFocus();">
                        ...
                        <div class="miss-header">
                            ...
                            <div class="header-right-buttonbtn">
                                <div class="buttonbtn-group">
                                    <div class="buttonbtn new-btn"><a>新建</a></div>
                                </div>
                            </div>
                            <div class="header-right-buttonbtn">
    ...
    <script>
        function formatOper(val, row, index, entityType) {
            var rowId = row["LIST_RECORD_ID"]; //return '<a style=\"color:#015ba7;font-size:13px;\" href="e?id=' + rowId + '">修改</a>';
            var html = '<a onclick=editrow("' + rowId + '") style=\"color:#015ba7;font-size:13px;\">编辑</a>&nbsp;';
            html += '&nbsp|&nbsp<a onclick=deleterow("' + rowId + '","' + webContext.objectTypeCode + '") style=\"color:#015ba7;font-size:13px;\">删除</a>&nbsp;';
           return html
        }
        function deleterow(id, type) {
            $('.popup-mask').show()
            $('#iframe').attr('src', '/alert/delete_ie8.html?id=' + id + '&objtypecode='+type).show().height(280).css('margin-top', '-140px')

        }
        function editrow(id) {
            $('.popup-mask').show()
            $('#iframe').attr('src', '/layouteditor/add_layout_ie8.html?id=' + id + '&entityName=' + webContext.entityApiName).show().height(620).css('margin-top', '-310px')
        }
